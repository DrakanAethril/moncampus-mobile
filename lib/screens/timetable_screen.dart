...
class _TimetableScreenState extends State<TimetableScreen> {
  ...
  static const _weekdayShort = ['lun', 'mar', 'mer', 'jeu', 'ven', 'sam', 'dim'];
  static const _weekdayFull = ['lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi', 'dimanche'];
  ...
  Future<void> _loadWeek() async {
    ...
    try {
      final sessions = await _timetableService.fetchWeek(token, from: _weekStart, to: _weekStart.add(const Duration(days: 6)));
      if (mounted) {
        setState(() {
  ...
  List<LessonSession> get _daySessions {
    ...
    final filtered = sessions.where((session) => _isSameDay(session.day, _selectedDay)).toList()
    ...
    return Container(
      ...
      child: Column(
        children: [
          const Row(
            children: [
              Text('Emploi du temps', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 17)),
            ],
          ),
                            ...
                            children: [
                              Text(
                                ...
                                style: TextStyle(
                                  ...
                                  color: isSelected ? AppColors.navy : const Color(0xFF7D99B0),
                                ),
                              ),
                              Text(
                                '${day.day}',
  ...
  Widget _buildBody() {
    ...
    final summary = '${_weekdayFull[_selectedDay.weekday - 1]} ${_selectedDay.day} ${_monthNames[_selectedDay.month - 1]}'
        ' · ${sessions.length} cours';

    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        Text(summary.toUpperCase(), style: const TextStyle(fontSize: 12, color: AppColors.faint, fontWeight: FontWeight.w600)),
        const SizedBox(height: 10),
        if (sessions.isEmpty)
          const Padding(
            padding: EdgeInsets.symmetric(vertical: 40),
            child: Center(child: Text('Aucun cours ce jour-là.', style: TextStyle(color: AppColors.faint))),
          )
        else
      ...
      child: Row(
        children: [
          ...
          Padding(
            ...
            child: Text('Pause déjeuner', style: TextStyle(fontSize: 11.5, color: AppColors.faint)),
          ),
          Expanded(child: Divider(color: AppColors.border)),
        ],
      ),
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            ...
            child: Text(
              '${session.startTime}\n${session.endTime}',
              style: TextStyle(fontFamily: 'monospace', fontWeight: FontWeight.w600, fontSize: 12, color: color, height: 1.5),
            ),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(session.title, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14.5, color: AppColors.ink)),
